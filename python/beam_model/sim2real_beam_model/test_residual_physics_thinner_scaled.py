...
def test_trajectory(
    cantilever:CantileverEnv3d, save_folder, test_data_idx, transformed_markers, start_frame=0, end_frame=150, cantilever_sim=None, cantilever_sim_2 = None, default_cantilever=None
):
    ...
    q0 = torch.from_numpy(np.load(f"cantilever_data_thinner_scaled_straight/q_force_opt{test_data_idx}_reorder.npz")['arr_%d' % (len(np.load(f"cantilever_data_thinner_scaled_straight/q_force_opt{test_data_idx}_reorder.npz"))-1)])
    ...
    np.save(f"{save_folder}/thinner_scaled/qs_origin_{test_data_idx}.npy", qs_origin)
    np.save(f"{save_folder}/thinner_scaled/vs_origin_{test_data_idx}.npy", vs_origin)
    ...
    vis_1d_folder = f"thinner_scaled_2dplots_displacement/{save_folder.replace('training/', '')}_residual_network"
...
if __name__ == "__main__":
    ...
    save_folder =  f"training/test_refactor" #_element_direct"
    sim_errors = []
    sim_2_errors = []
    res_errors = []
    origin_errors = []
    ...
    for test_i in [2,7,11,14,16]: 
        ...
        cantilever_sim = ThinnerCantileverEnv3d(42, 'beam_thinner_scaled', hex_params)
        ...
        if 'longer' not in save_folder and 'shorter' not in save_folder and 'thicker' not in save_folder and 'thinner' not in save_folder:
            ...
            cantilever_sim_2 = ThinnerCantileverEnv3d(42, 'beam_thinner_scaled', hex_params)
        else:
        ...
        qs_real = np.load(f"data_thinner/q{test_i}.npy")
        ...
        sim_error, sim_2_error, res_error, time_sim, time_res, time_network, origin_error, time_origin = test_trajectory(
            cantilever, save_folder, test_i, transformed_markers, end_frame=140, cantilever_sim=cantilever_sim, cantilever_sim_2 = cantilever_sim_2, default_cantilever=default_cantilever
        )
    ...
    np.save(f"{save_folder}/thinner_scaled/sim_errors_residual_network.npy", sim_errors)
    np.save(f"{save_folder}/thinner_scaled/res_errors_residual_network.npy", res_errors)
    np.save(f"{save_folder}/thinner_scaled/origin_errors_residual_network.npy", origin_errors)