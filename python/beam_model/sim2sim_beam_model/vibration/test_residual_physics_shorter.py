...
def test_trajectory(
    cantilever:ShorterCantileverEnv3d, save_folder, test_data_idx,  start_frame=0, end_frame=150, cantilever_sim=None,default_cantilever=None
):
    ...
    ground_truth = np.load(
        f"shorter_cantilever_data_sim2sim/optimized_data_{test_data_idx}.npy",
        allow_pickle=True,
    )[()]
    ...
    vis_1d_folder = f"shorter_sim2sim/{save_folder.replace('training/', '')}_{model_input}"
...
if __name__ == "__main__":
    ...
    cantilever = ShorterCantileverEnv3d(42, 'beam', hex_params)
    ...
    np.save(f"{save_folder}/shorter/res_errors_residual_network.npy", sim_errors)
