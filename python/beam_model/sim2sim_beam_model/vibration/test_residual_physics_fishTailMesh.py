...
def test_trajectory(
    ...
    np.save(f"{save_folder}/fishTailMesh/qs_sim_{test_data_idx}.npy", qs_sim)
    np.save(f"{save_folder}/fishTailMesh/qs_res_{test_data_idx}.npy", qs_res)
    np.save(f"{save_folder}/fishTailMesh/vs_sim_{test_data_idx}.npy", vs_sim)
    np.save(f"{save_folder}/fishTailMesh/vs_res_{test_data_idx}.npy", vs_res)
    ...
    vis_1d_folder = f"fishTailMesh_sim2sim/{save_folder.replace('training/', '')}_{model_input}"
...
if __name__ == "__main__":
    ...
    default_cantilever = CantileverEnv3d(42, 'fishTailMesh', hex_params)
    ...
    np.save(f"{save_folder}/fishTailMesh/res_errors_residual_network.npy", sim_errors)
    np.save(f"{save_folder}/fishTailMesh/res_errors_residual_network.npy", res_errors)

    # generate_video_directory(f"{save_folder}/fishTailMesh/visualizations/residual", [2,7,11,14,16], flag="")
    # generate_video_directory(f"{save_folder}/fishTailMesh/visualizations/base", [2,7,11,14,16], flag="")
