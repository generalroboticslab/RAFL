...
#include "pd_energy/meta_volume_pd_element_energy.h"
...
void Deformable<vertex_dim, element_dim>::AddPdEnergy(const std::string& energy_type, const std::vector<real>& params,
    ...
    else if (energy_type == "meta_corotated") {
        CheckError(param_size == 2, "Inconsistent param size.");
        auto energy = std::make_shared<MetaCorotatedPdElementEnergy<vertex_dim>>();
        const real stiffness_1 = params[0];
        const real stiffness_2 = params[1];
        energy->Initialize(stiffness_1, stiffness_2);

        CheckError(indices.empty(), "Meta Corotated PD material is assumed to be applied to all elements.");

        pd_element_energies_.push_back(energy);

        meta_material_ = true;
    } else if (energy_type == "meta_volume") {
        CheckError(param_size == 2, "Inconsistent param size.");
        auto energy = std::make_shared<MetaVolumePdElementEnergy<vertex_dim>>();
        const real stiffness_1 = params[0];
        const real stiffness_2 = params[1];
        energy->Initialize(stiffness_1, stiffness_2);

        CheckError(indices.empty(), "Meta Volume PD material is assumed to be applied to all elements.");
...
template<int vertex_dim, int element_dim>
const real Deformable<vertex_dim, element_dim>::ComputePdEnergy(const VectorXr& q, const VectorXr& p, const bool use_precomputed_data) const {
    real total_energy = 0;
    for (const auto& pair : pd_vertex_energies_) {
        const auto& energy = pair.first;
        for (const int idx : pair.second) {
    ...
    for (const auto& energy : pd_element_energies_) {
        ...
        for (int i = 0; i < element_num; ++i) {
            ...
            for (int j = 0; j < sample_num; ++j) {
                ...
                if (use_precomputed_data)
                    element_energy[i] += energy->EnergyDensity(F_auxiliary_[i][j],
                        projections_[energy_cnt][i][j], p[i]) * element_volume_ / sample_num;
                else
                    element_energy[i] += energy->EnergyDensity(F, p[i]) * element_volume_ / sample_num;
            }
        }
        ++energy_cnt;
    }
...
template<int vertex_dim, int element_dim>
const VectorXr Deformable<vertex_dim, element_dim>::PdEnergyForce(const VectorXr& q, const VectorXr& p, const bool use_precomputed_data) const {
    VectorXr f = VectorXr::Zero(dofs_);
    for (const auto& pair : pd_vertex_energies_) {
        const auto& energy = pair.first;
        for (const int idx : pair.second) {
    ...
    for (const auto& energy : pd_element_energies_) {
        ...
        for (int i = 0; i < element_num; ++i) {
            ...
            for (int j = 0; j < sample_num; ++j) {
                ...
                Eigen::Matrix<real, vertex_dim, vertex_dim> P;
                if (use_precomputed_data)
                    P = energy->StressTensor(F_auxiliary_[i][j], projections_[energy_cnt][i][j], p[i]);
                else
                    P = energy->StressTensor(F, p[i]);
                const Eigen::Matrix<real, 1, vertex_dim * element_dim> f_kd =
                    -Flatten(P).transpose() * finite_element_samples_[i][j].dF_dxkd_flattened() * element_volume_ / sample_num;
                for (int k = 0; k < element_dim; ++k) {
                    f_ints[i * element_dim + k] += Eigen::Matrix<real, vertex_dim, 1>(f_kd.segment(k * vertex_dim, vertex_dim));
...
template<int vertex_dim, int element_dim>
const VectorXr Deformable<vertex_dim, element_dim>::PdEnergyForceDifferential(const VectorXr& q, const VectorXr& p, const VectorXr& dq, const VectorXr& dp,
    const VectorXr& dw) const {
    CheckError(static_cast<int>(dw.size()) == static_cast<int>(pd_element_energies_.size()), "Inconsistent length of dw.");
    VectorXr df = VectorXr::Zero(dofs_);
    for (const auto& pair : pd_vertex_energies_) {
    ...
    for (const auto& energy : pd_element_energies_) {
        #pragma omp parallel for
        for (int i = 0; i < element_num; ++i) {
            const real inv_w = 1 / energy->stiffness(p[i]);
            const auto deformed = ScatterToElement(q, i);
            const auto ddeformed = ScatterToElement(dq, i);
            for (int j = 0; j < sample_num; ++j) {
                const auto F = DeformationGradient(i, deformed, j);
                ...
                const int w_idx = 2 * energy_cnt + static_cast<int>(std::round(p[i]));

                const Eigen::Matrix<real, vertex_dim, vertex_dim> dP = energy->StressTensorDifferential(F, dF, p[i])
                    + energy->StressTensor(F, p[i]) * inv_w * dw(w_idx);
...
template<int vertex_dim, int element_dim>
void Deformable<vertex_dim, element_dim>::PdEnergyForceDifferential(const VectorXr& q, const VectorXr& p, const bool require_dq, 
    const bool require_dw, const bool use_precomputed_data, SparseMatrixElements& dq, SparseMatrixElements& dp, SparseMatrixElements& dw) const {
    dq.clear();
    dp.clear();
    dw.clear();
    for (const auto& pair : pd_vertex_energies_) {
        ...
        for (const auto& energy : pd_element_energies_) {
            for (int i = 0; i < element_num; ++i) {
                ...
                for (int j = 0; j < sample_num; ++j) {
                    ...
                    Eigen::Matrix<real, vertex_dim * vertex_dim, element_dim * vertex_dim> dP_from_dF;
                    if (use_precomputed_data){
                        dP_from_dF = energy->StressTensorDifferential(F_auxiliary_[i][j], projections_[energy_cnt][i][j], p[i]) * dF;
                    }
                    else{
                        dP_from_dF = energy->StressTensorDifferential(DeformationGradient(i, deformed, j), p[i]) * dF;
                    }
                    const Eigen::Matrix<real, element_dim * vertex_dim, element_dim * vertex_dim> df_kd_from_dF
                        = -dP_from_dF.transpose() * finite_element_samples_[i][j].dF_dxkd_flattened() * element_volume_ / sample_num;
                    const int offset = dq_offset + (energy_cnt * element_num * sample_num + i * sample_num + j)
    ...
    if (require_dw) {
        int energy_cnt = 0;
        dw.resize(pd_element_energies_.size() * element_num * sample_num * element_dim * vertex_dim);
        dp.resize(pd_element_energies_.size() * element_num * sample_num * element_dim * vertex_dim);
        for (const auto& energy : pd_element_energies_) {
            const int offset = energy_cnt * element_num * sample_num * element_dim * vertex_dim;
            #pragma omp parallel for
            for (int i = 0; i < element_num; ++i) {
                const real inv_w = 1 / energy->stiffness(p[i]);
                const real delta_w = energy->delta_stiffness();
                ...
                for (int j = 0; j < sample_num; ++j) {
                    ...
                    if (use_precomputed_data) {
                        ...
                        P = energy->StressTensor(DeformationGradient(i, deformed, j), p[i]);
                    }
                    const auto dP_from_dw = Flatten(P) * inv_w;
                    const Eigen::Matrix<real, 1, element_dim * vertex_dim> df_kd_from_dw
                        = -dP_from_dw.transpose() * finite_element_samples_[i][j].dF_dxkd_flattened() * element_volume_ / sample_num;
                    ...
                    for (int k = 0; k < element_dim; ++k)
                        for (int d = 0; d < vertex_dim; ++d) {
                            const int idx = offset + i * sample_num * element_dim * vertex_dim
                                + j * element_dim * vertex_dim + k * vertex_dim + d;
                            dw[idx] = Eigen::Triplet<real>(vertex_dim * vi(k) + d,
                                w_idx, df_kd_from_dw(k * vertex_dim + d));
                            dp[idx] = Eigen::Triplet<real>(vertex_dim * vi(k) + d,
                                i, delta_w * df_kd_from_dw(k * vertex_dim + d));
                        }
                }
            }
            ++energy_cnt;
...
template<int vertex_dim, int element_dim>
const real Deformable<vertex_dim, element_dim>::PyComputePdEnergy(const std::vector<real>& q, const std::vector<real>& p) const {
    return ComputePdEnergy(ToEigenVector(q), ToEigenVector(p), false);
}

template<int vertex_dim, int element_dim>
const std::vector<real> Deformable<vertex_dim, element_dim>::PyPdEnergyForce(const std::vector<real>& q, const std::vector<real>& p) const {
    return ToStdVector(PdEnergyForce(ToEigenVector(q), ToEigenVector(p), false));
}

template<int vertex_dim, int element_dim>
const std::vector<real> Deformable<vertex_dim, element_dim>::PyPdEnergyForceDifferential(const std::vector<real>& q, const std::vector<real>& p,
    const std::vector<real>& dq, const std::vector<real>& dp, const std::vector<real>& dw) const {
    return ToStdVector(PdEnergyForceDifferential(ToEigenVector(q), ToEigenVector(p), ToEigenVector(dq), ToEigenVector(dp), ToEigenVector(dw)));
}

template<int vertex_dim, int element_dim>
void Deformable<vertex_dim, element_dim>::PyPdEnergyForceDifferential(
    const std::vector<real>& q, const std::vector<real>& p, const bool require_dq, const bool require_dw,
    std::vector<std::vector<real>>& dq, std::vector<std::vector<real>>& dp, std::vector<std::vector<real>>& dw) const {
    PrintWarning("PyPdEnergyForceDifferential should only be used for small-scale problems and for testing purposes.");
    SparseMatrixElements nonzeros_q, nonzeros_p, nonzeros_w;
    PdEnergyForceDifferential(ToEigenVector(q), ToEigenVector(p), require_dq, require_dw, false, nonzeros_q, nonzeros_p, nonzeros_w);
    dq.resize(dofs_);
    dp.resize(dofs_);
    dw.resize(dofs_);
    const int p_dofs = mesh_.NumOfElements();
    const int w_dofs = 2 * static_cast<int>(pd_element_energies_.size());
    for (int i = 0; i < dofs_; ++i) {
        dq[i].resize(dofs_);
        dp[i].resize(p_dofs);
        dw[i].resize(w_dofs);
    ...
    if (require_dw) {
        for (const auto& triplet : nonzeros_p) {
            dp[triplet.row()][triplet.col()] += triplet.value();
        }
        for (const auto& triplet : nonzeros_w) {
            dw[triplet.row()][triplet.col()] += triplet.value();
        }
