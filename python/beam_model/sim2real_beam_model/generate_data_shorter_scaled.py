...
def optimize_init_force(
    ...
    q_all = []
    print(f"test{id}")
    path_vis = Path(f"cantilever_data_shorter_scaled_{suffix}/test{id}/init_beam")
    ...
    with torch.no_grad():
        ...
        Path(f"cantilever_data_shorter_scaled_{suffix}").mkdir(exist_ok=True, parents=True)
        np.savez(f"cantilever_data_shorter_scaled_{suffix}/q_force_opt{id}_reorder.npz", *q_all)
        np.save(f"cantilever_data_shorter_scaled_{suffix}/q_force_opt{id}_init.npy", params.detach().cpu().numpy())
...
if __name__ == '__main__':
    ...
    for idx in idxs:
        ...
        cantilever = ShorterCantileverEnv3d(42, f'cantilever_data_shorter_scaled_straight/test{idx}', hex_params)
