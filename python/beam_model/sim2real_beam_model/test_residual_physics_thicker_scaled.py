...
def test_trajectory(
    ...
    q0 = torch.from_numpy(np.load(f"cantilever_data_thicker_scaled_straight/q_force_opt{test_data_idx}_reorder.npz")['arr_%d' % (len(np.load(f"cantilever_data_thicker_scaled_straight/q_force_opt{test_data_idx}_reorder.npz"))-1)])
    ...
    np.save(f"{save_folder}/thicker_scaled/qs_origin_{test_data_idx}.npy", qs_origin)
    np.save(f"{save_folder}/thicker_scaled/vs_origin_{test_data_idx}.npy", vs_origin)
    ...
    vis_1d_folder = f"thicker_scaled_2dplots_displacement/{save_folder.replace('training/', '')}_residual_network"
...
if __name__ == "__main__":
    ...
    for test_i in [2,7,11,14,16]: 
        ...
        cantilever_sim = ThickerCantileverEnv3d(42, 'beam_thicker_scaled', hex_params)
        ...
        if 'longer' not in save_folder and 'shorter' not in save_folder and 'thicker' not in save_folder and 'thinner' not in save_folder:
            ...
            cantilever_sim_2 = ThickerCantileverEnv3d(42, 'beam_thicker_scaled', hex_params)
        else:
        ...
        qs_real = np.load(f"data_thicker/q{test_i}.npy")
