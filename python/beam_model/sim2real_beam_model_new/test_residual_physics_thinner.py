...
def test_trajectory(
    ...
    q0 = torch.from_numpy(np.load(f"cantilever_data_thinner_straight/q_force_opt{test_data_idx}_reorder.npz")['arr_%d' % (len(np.load(f"cantilever_data_thinner_straight/q_force_opt{test_data_idx}_reorder.npz"))-1)])
    ...
    for frame_i in range(1, end_frame):
        ...
        if cantilever_sim_2 is not None:
            qs_sim_2.append(q_sim_2.detach().numpy())
            vs_sim_2.append(v_sim_2.detach().numpy())
    np.save(f"{save_folder}/thinner/qs_sim_{test_data_idx}.npy", qs_sim)
    np.save(f"{save_folder}/thinner/qs_res_{test_data_idx}.npy", qs_res)
    np.save(f"{save_folder}/thinner/vs_sim_{test_data_idx}.npy", vs_sim)
    np.save(f"{save_folder}/thinner/vs_res_{test_data_idx}.npy", vs_res)
    np.save(f"{save_folder}/thinner/qs_origin_{test_data_idx}.npy", qs_origin)
    np.save(f"{save_folder}/thinner/vs_origin_{test_data_idx}.npy", vs_origin)
    ...
    vis_1d_folder = f"thinner_2dplots_displacement/{save_folder.replace('training/', '')}_residual_network"
...
if __name__ == "__main__":
    ...
    os.makedirs(f"beam_thinner/{save_folder.replace('training/', '')}", exist_ok=True)
    os.makedirs(f"beam_thinner/{save_folder.replace('training/', '')}", exist_ok=True)
    ...
    for test_i in [2,7,11,14,16]: 
        ...
        cantilever_sim = ThinnerCantileverEnv3d(42, 'beam_thinner', hex_params)
        ...
        if 'longer' not in save_folder and 'shorter' not in save_folder and 'thicker' not in save_folder and 'thinner' not in save_folder:
            ...
            cantilever_sim_2 = ThinnerCantileverEnv3d(42, 'beam_thinner', hex_params)
        else:
    ...
    np.save(f"{save_folder}/thinner/sim_errors_residual_network.npy", sim_errors)
    np.save(f"{save_folder}/thinner/res_errors_residual_network.npy", res_errors)
    np.save(f"{save_folder}/thinner/origin_errors_residual_network.npy", origin_errors)
