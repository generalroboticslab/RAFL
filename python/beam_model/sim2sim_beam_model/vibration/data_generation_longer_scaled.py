...
if __name__ == '__main__':
    ...
    prepare = 200
    save_folder = 'longer_scaled_data_real' 
    ...
    for id in range(len(weights)):
        ...
        # cantilever.display_mesh(q.detach(), f"{save_folder}/visualizations/{id}/origin.png")
        # print("Q x max: ", q.reshape(-1,3).max(dim=0)[0][0])
        for j in range(prepare):
            ...
            # print("Q x max: ", q.reshape(-1,3).max(dim=0)[0][0])
            # cantilever.display_mesh(q.detach(), f"{save_folder}/visualizations/{id}/{j}.png")
