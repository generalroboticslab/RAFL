...
def test_trajectory(
    ...
    q0 = torch.from_numpy(np.load(f"cantilever_data_thicker_straight/q_force_opt{test_data_idx}_reorder.npz")['arr_%d' % (len(np.load(f"cantilever_data_thicker_straight/q_force_opt{test_data_idx}_reorder.npz"))-1)])
    ...
    np.save(f"{save_folder}/thicker/vs_res_{test_data_idx}.npy", vs_res)
    np.save(f"{save_folder}/thicker/qs_origin_{test_data_idx}.npy", qs_origin)
    np.save(f"{save_folder}/thicker/vs_origin_{test_data_idx}.npy", vs_origin)
    ...
    vis_1d_folder = f"thicker_2dplots_displacement/{save_folder.replace('training/', '')}_residual_network"
...
if __name__ == "__main__":
    ...
    os.makedirs(f"beam_thicker/{save_folder.replace('training/', '')}", exist_ok=True)
    os.makedirs(f"beam_thicker/{save_folder.replace('training/', '')}", exist_ok=True)
    ...
    for test_i in [2,7,11,14,16]: 
        ...
        cantilever = ThickerCantileverEnv3d(42, 'beam_thicker', hex_params)

        hex_params['youngs_modulus'], hex_params['poissons_ratio'] = get_test_params(save_folder)
        
        cantilever_sim = ThickerCantileverEnv3d(42, 'beam_thicker', hex_params)
        ...
        if 'longer' not in save_folder and 'shorter' not in save_folder and 'thicker' not in save_folder and 'thinner' not in save_folder:
            ...
            cantilever_sim_2 = ThickerCantileverEnv3d(42, 'beam_thicker', hex_params)
        else:
    ...
    np.save(f"{save_folder}/thicker/sim_errors_residual_network.npy", sim_errors)
    np.save(f"{save_folder}/thicker/res_errors_residual_network.npy", res_errors)
    np.save(f"{save_folder}/thicker/origin_errors_residual_network.npy", origin_errors)