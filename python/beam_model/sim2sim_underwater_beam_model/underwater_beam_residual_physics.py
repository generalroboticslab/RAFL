...
class UnderwaterCantileverResidualPhysics(ResidualPhysicsBase):
    def __init__(self, config, folder, options):
        super().__init__(config)
        self.loss_fn = nn.MSELoss(reduction="mean")
        self.diffpd_model = UnderwaterCantileverEnv3d(config['seed'], folder, options)

    def train(self, data_path, config):
        diffpd_model = self.diffpd_model
        if config["model"] == "skip_connection":
            self.residual_network = ResMLPResidual2(diffpd_model._dofs * 3, diffpd_model._dofs, hidden_size=config['hidden_size'], num_mlp_blocks=config['num_mlp_blocks'], num_block_layer=config['num_hidden_layer'])
        elif config["model"] == "MLP":
            self.residual_network = MLPResidual(diffpd_model._dofs*3, diffpd_model._dofs)
            ...
            for epoch in range(self.epochs):
                ...
                for (
                    ...
                    residual_forces = self.residual_network(
                        torch.cat(
                            (q_start_batch, v_start_batch, p_batch), dim=1
                        )
                    )
