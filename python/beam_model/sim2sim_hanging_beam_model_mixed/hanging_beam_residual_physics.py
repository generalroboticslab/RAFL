...
class HangingCantileverResidualPhysics(ResidualPhysicsBase):
    def __init__(self, config, folder, options, target_dx=0.01):
        super().__init__(config)
        self.loss_fn = nn.MSELoss(reduction="mean")
        self.diffpd_model = HangingCantileverEnv3d(config['seed'], folder, options)

    def train(self, data_path, config):
        diffpd_model = self.diffpd_model

        self.boundary_indices = []
        self.nonboundary_indices = []
        for i in range(0,diffpd_model._dofs,3):
            if diffpd_model.is_dirichlet_dof(i):
                self.boundary_indices.extend([i,i+1,i+2])
            else:
                self.nonboundary_indices.extend([i,i+1,i+2])
        ...
        if config["model"] == "skip_connection":
            ...
            youngs_modulus = self.diffpd_model._youngs_modulus
            poissons_ratio = self.diffpd_model._poissons_ratio 
            la = (
                youngs_modulus
                * poissons_ratio
            ...
            # print(mesh.dx())
            # print(diffpd_model._deformable.density())
            self.residual_network = ElementResidual(diffpd_model._dofs, 
                                                    ...
                                                    gravity = True,
                                                    changing_boundary_indices=self.boundary_indices
                                                    )
        elif config["model"] == "implicit":
            ...
            print(mesh.dx())
            print(diffpd_model._deformable.density())
            self.residual_network = ImplicitResidual(diffpd_model._dofs, 
                                                    torch.tensor(elements), 
                                                    torch.tensor(rho), 
                                                    diffpd_model.q0, 
                                                    mesh.dx(),
        ...
        if self.validation:
            ...
            validation_set = UnderwaterCantileverDataset(
    ...
    def fit_residual_physics(
        ...
        f_mean, f_std = torch.zeros(training_set.q_init.shape[0]).flatten(), torch.std(training_set.fs[:,:,self.nonboundary_indices].view(-1,3), dim=0).expand(training_set.q_init.shape[0] // 3, 3).flatten()
        device = self.device
            ...
            for epoch in range(self.epochs):
                ...
                for (
                    ...
                    residual_forces = self.residual_network(
                        torch.cat(
                            (q_start_batch, v_start_batch), dim=1 #, p_batch), dim=1
                        )
                    )
                    loss = self.loss_fn(residual_forces[:,self.nonboundary_indices], f_optimized[:,self.nonboundary_indices]) * self.scaling
                    loss.backward()
                ...
                if self.validation:
                    ...
                    for (
                        ...
                        residual_forces = self.residual_network(
                            torch.cat(
                                (q_start_batch, v_start_batch), #, p_batch),
                                dim=1,
                            )
                        )
                        f_ext_loss = self.loss_fn(residual_forces[:,self.nonboundary_indices], f_optimized[:,self.nonboundary_indices]) * self.scaling
