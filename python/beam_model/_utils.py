...
class UnderwaterCantileverDataset(torch.utils.data.Dataset):
    def __init__(self, data_set, q_init, path, start_frame=0, end_frame=149):
        ...
        for i, data_idx in enumerate(data_set):
            ...
            p = torch.from_numpy(data_i["pressure_forces"])[start_frame:end_frame-1, :]
            q = q - self.q_init
            qs[i] = q
            vs[i] = v
            fs[i] = f.t()
