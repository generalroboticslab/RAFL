...
def optimize_trajectoryfull( hex_params, num_frames, num_epochs, dt, sample=0, suffix = "solid"):

    target_data = np.load(f"data_real_{suffix}/trajectory{sample}.npy", allow_pickle=True)[()]
    hex_params["amplitude"] = target_data["amplitude"]
    
    cantilever = UnderwaterCantileverEnv3d(42, f'oscillate_base', hex_params)
    
    target_trajectory = torch.from_numpy(target_data['q'])
    q0 = target_trajectory[0]
    v0 = torch.zeros(cantilever._dofs, dtype=torch.float64)

    if suffix == 'solid':
        Path(f"oscillate_base/{sample}").mkdir(parents=True, exist_ok=True)

        q, v = q0.clone(), v0.clone()
        cantilever.display_mesh(q, f"oscillate_base/{sample}/0.png")
        for k in range(int(num_frames)):
            ...
            file_name = f"oscillate_base/{sample}/{k+1}.png"
    ...
    for frame_i in range(1, num_frames):
        ...
        with torch.no_grad():
            ...
            for node_dof in range(0, cantilever._dofs, 3):
                if cantilever.is_dirichlet_dof(node_dof):
                    q_last_epoch[node_dof] = q0[node_dof] + vx_offset
                    q_last_epoch[node_dof + 1] = q0[node_dof + 1]
                    q_last_epoch[node_dof + 2] = q0[node_dof + 2]
    ...
    Path(f"cantilever_data_sim2sim_{suffix}").mkdir(parents=True,exist_ok=True)
    np.save(f"cantilever_data_sim2sim_{suffix}/optimized_data_{sample}.npy", optimized_data_save)
...
if __name__ == '__main__':
    ...
    for idx in idxs:
        ...
        optimize_trajectoryfull(hex_params, num_frames, num_epochs, 1e-2, sample=idx, suffix="solid")
    
    generate_video_directory("oscillate_base", idxs, flag="")