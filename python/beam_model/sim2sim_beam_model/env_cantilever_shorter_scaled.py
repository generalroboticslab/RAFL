...
class ShorterCantileverEnv3d (EnvBase):
    ...
    def __init__(self, seed, folder, options):
        ...
        ### Mesh Parameters
        # Cantilever is 0.08m long, 0.03m wide, and 0.03m tall
        dx = ndarray([0.008 / refinement, 0.01 / refinement, 0.01 / refinement])
        cell_nums = (int(round(0.08 / dx[0])), int(round(0.03 / dx[1])), int(round(0.03 / dx[2])))
        assert cell_nums[0] * dx[0] == 0.08 and cell_nums[1] * dx[1] == 0.03, "Refinement does not properly divide the cantilever dimensions!" 
