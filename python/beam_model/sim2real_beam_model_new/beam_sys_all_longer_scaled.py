...
def system_identification ( trajectories, num_frames, dt, verbose=False, folder="sysID"):
    ...
    def loss_and_grad (x):
        ...
        for traj_i in trajectories:
            ...
            # q0 = qs[0]
            q0 = np.load(f"cantilever_data_longer_scaled_straight/q_force_opt{traj_i}_reorder.npz")['arr_%d' % (len(np.load(f"cantilever_data_longer_scaled_straight/q_force_opt{traj_i}_reorder.npz"))-1)]
            v0 = np.zeros_like(q0)
...
def run_simulation (trajectories, youngs_modulus, poisson_ratio, dt=0.01, folder="sysID"):
    ...
    for idx, traj_i in enumerate(trajectories):
        ...
        q = torch.from_numpy(np.load(f"cantilever_data_longer_scaled_straight/q_force_opt{traj_i}_reorder.npz")['arr_%d' % (len(np.load(f"cantilever_data_longer_scaled_straight/q_force_opt{traj_i}_reorder.npz"))-1)])
        v = torch.zeros_like(q)
