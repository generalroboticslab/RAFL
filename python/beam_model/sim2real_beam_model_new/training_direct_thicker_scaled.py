...
def train(
    cantilever:ThickerCantileverEnv3d, save_folder, start_frame=0, end_frame=150, num_epochs=100, cantilever_sim=None, 
):
    ...
    with tqdm(num_epochs) as qbar:
        for epoch in range(num_epochs):
            ...
            for data_idx in training_options["training_set"]:
                qs_real = np.load(f'data_thicker/q{data_idx}.npy' )
                ...
                q_arr = np.load(f"cantilever_data_thicker_scaled_straight/q_force_opt{data_idx}_reorder.npz")['arr_%d' % (len(np.load(f"cantilever_data_thicker_scaled_straight/q_force_opt{data_idx}_reorder.npz"))-1)]
...
if __name__ == "__main__":
    ...
    cantilever = ThickerCantileverEnv3d(42, 'thicker_scaled_beam', hex_params)
    q_init = torch.from_numpy(cantilever._q0)

    save_folder = f"training/test_refactor_element_nonWeighted_try_unseparated_unconditioned_thicker_scaled_direct"
