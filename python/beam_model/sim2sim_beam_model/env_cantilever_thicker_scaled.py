...
class ThickerCantileverEnv3d (EnvBase):
    ...
    def __init__(self, seed, folder, options):
        ...
        ### Mesh Parameters
        # Cantilever is 0.10m long, 0.04m wide, and 0.04m tall
        dx = ndarray([0.01 / refinement, 0.01333333 / refinement, 0.01333333 / refinement])
        cell_nums = (int(round(0.1 / dx[0])), int(round(0.04 / dx[1])), int(round(0.04 / dx[2])))
        assert cell_nums[0] * dx[0] == 0.1 and np.isclose(cell_nums[1] * dx[1],0.04), "Refinement does not properly divide the cantilever dimensions!" 
        ...
        for i in range(1, node_nums[0]):
            ...
            center = (
                ndarray([i * dx[0], cell_nums[1] / 2 * dx[1], cell_nums[2] / 2 * dx[2]]) + origin
            )
