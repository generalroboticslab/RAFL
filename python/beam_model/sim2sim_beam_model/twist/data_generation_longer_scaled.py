...
def data_generate(sample_num, frame_num, hex_params, vis=True):
    seed = 42
    folder = "longer_scaled_data_real"
    refinement = 1
    ...
    for idx, twist_angle in enumerate(twist_angle_range):
        ...
        data_folder = 'longer_scaled_data_real'
        Path(f"{data_folder}").mkdir(parents=True, exist_ok=True)
        np.save(f"{data_folder}/trajectory{idx}.npy", data)

        Path(f"{data_folder}/visualizations/{idx}").mkdir(parents=True, exist_ok=True)
        for k, q in enumerate(qv["q"]):
            env.display_mesh(q, f"{data_folder}/visualizations/{idx}/{k}.png")
...
if __name__ == "__main__":
    ...
    data_generate(sample_num, frame_num, hex_params, vis=True)
    generate_video_directory(f"longer_scaled_data_real/visualizations", list(range(sample_num)), flag="")