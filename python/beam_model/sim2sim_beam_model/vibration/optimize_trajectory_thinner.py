...
def optimize_trajectoryfull(cantilever:ThinnerCantileverEnv3d, num_frames, num_epochs, dt, sample=0, suffix = "fix_registration"):
    ...
    target_trajectory_q = np.load(f"thinner_data_real/trajectory{sample}.npy", allow_pickle=True)[()]['q']
    target_trajectory_v = np.load(f"thinner_data_real/trajectory{sample}.npy", allow_pickle=True)[()]['v']
    ...
    Path(f"thinner_cantilever_data_{suffix}").mkdir(parents=True,exist_ok=True)
    np.save(f"thinner_cantilever_data_{suffix}/optimized_data_{sample}.npy", optimized_data_save)
...
if __name__ == '__main__':
    ...
    for idx in idxs:
        ...
        cantilever = ThinnerCantileverEnv3d(42, 'thinner_data_real', hex_params)
