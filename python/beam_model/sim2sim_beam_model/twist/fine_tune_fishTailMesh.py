...
def fine_tune(
    cantilever:FishTailMeshEnv3d, save_folder, start_frame=0, end_frame=150, num_epochs=100, cantilever_sim=None, default_cantilever=None
):

    if cantilever_sim is None:
        cantilever_sim = cantilever
    training_options = yaml.safe_load(open(f"{save_folder}/config.yaml"))
    with open(f"{save_folder}/fishTailMesh_finetune/config.yaml", 'w') as f:
    ...
    with tqdm(num_epochs) as qbar:
        for epoch in range(num_epochs):
            ...
            for data_idx in training_options["training_set"]:
                target_trajectory_q = torch.from_numpy(np.load(f"fishTailMesh_data_real/trajectory{data_idx}.npy", allow_pickle=True)[()]['q'])[0]
                target_trajectory_v = torch.from_numpy(np.load(f"fishTailMesh_data_real/trajectory{data_idx}.npy", allow_pickle=True)[()]['v'])[0]
            ...
            with torch.no_grad():
                ...
                if val_loss == np.min(val_loss_history):
                    torch.save(
                        ...
                        f"{save_folder}/fishTailMesh_finetune/residual_network.pth",
                    )
