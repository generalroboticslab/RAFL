...
def optimize_trajectoryfull(cantilever:ThinnerCantileverEnv3d, num_frames, num_epochs, dt, sample=0, suffix = "fix_registration"):
    ...
    target_trajectory_q = np.load(f"thinner_scaled_data_real/trajectory{sample}.npy", allow_pickle=True)[()]['q']
    target_trajectory_v = np.load(f"thinner_scaled_data_real/trajectory{sample}.npy", allow_pickle=True)[()]['v']
    ...
    print("length of q_all", target_trajectory_q.shape[0])
    print("length of v_all", target_trajectory_v.shape[0])
    optimized_data_save = {
        "q_trajectory": target_trajectory_q,
        "v_trajectory": target_trajectory_v,
        # "optimized_forces": params.detach().numpy(),
    }
    Path(f"thinner_scaled_cantilever_data_{suffix}").mkdir(parents=True,exist_ok=True)
    np.save(f"thinner_scaled_cantilever_data_{suffix}/optimized_data_{sample}.npy", optimized_data_save)
...
if __name__ == '__main__':
    ...
    for idx in idxs:
        ...
        cantilever = ThinnerCantileverEnv3d(42, 'thinner_scaled_data_real', hex_params)
