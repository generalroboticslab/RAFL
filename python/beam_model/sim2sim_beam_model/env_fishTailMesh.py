...
from py_diff_pd.common.tet_mesh import generate_tet_mesh, fix_tet_faces, tetrahedralize, read_tetgen_file
from py_diff_pd.core.py_diff_pd_core import TetMesh3d, TetDeformable

from py_diff_pd.common.hex_mesh import generate_hex_mesh, voxelize
from py_diff_pd.core.py_diff_pd_core import HexMesh3d, HexDeformable

class FishTailMeshEnv3d (EnvBase):
    ...
    def __init__(self, seed, folder, options):
        ...
        # tet_file = np.load('/home/dc424/residual_physics_sim2real/python/beam_model/sim2sim_beam_model/mesh/spot_0.05.npz')
        # verts, eles = tet_file['verts'], tet_file['elems']
        # verts, eles = tetrahedralize('/home/dc424/residual_physics_sim2real/python/beam_model/sim2sim_beam_model/mesh/rectangle.obj', normalize_input=False, options={'minratio': 1.1})
        
        # ele_file_name = '/home/dc424/residual_physics_sim2real/python/beam_model/sim2sim_beam_model/mesh/rectangle.1.ele'
        # node_file_name = '/home/dc424/residual_physics_sim2real/python/beam_model/sim2sim_beam_model/mesh/rectangle.1.node'
        # verts, eles = read_tetgen_file(node_file_name, ele_file_name)

        dx = 0.003

        tet_file = '/home/dc424/residual_physics_sim2real/python/beam_model/sim2sim_beam_model/mesh/fishtail.obj'
        voxels = voxelize(tet_file, dx=dx * 10)
        origin = ndarray([0.0, 0.0, 0.0])

        # To make the mesh consistent with our coordinate system, we need to:
        # - rotate the model along +z by -90 degrees.
        # - rotate the model along +x by 90 degrees.
        # - shift it so that its min_x = 0.
        # - divide it by 1000.
        # R = ndarray([
        #     [0, 1, 0],
        #     [-1, 0, 0],
        ...
        # ])
        # verts = verts @ R.T
        # # Next, rotate along x by -90 degrees.
        # R = ndarray([
        #     [1, 0, 0],
        #     [0, 0, -1],
        #     [0, 1, 0]
        ...
        for i in range(vert_num):
            ...
            elif abs(vx - max_corner[0]) < 1e-3:
                self.force_nodes.append(i)
        

        print("force_nodes: ", len(self.force_nodes))
        print("fixed_nodes: ", len(fixed_nodes))
        print("total_nodes: ", vert_num)
