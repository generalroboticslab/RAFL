...
from underwater_beam_residual_physics import UnderwaterCantileverResidualPhysics
...
if __name__ == "__main__":
    ...
    if ap.parse_args().config is not None:
        ...
        config["hidden_size"] = 256 #512
        config["actuated"] = True
        config["num_hidden_layer"] = 4 #3
        # save_folder = f"training/sim2simResMLP5"
        save_folder = f"training/test_refactor_element_liquid"
        config["data_folder"] = save_folder.replace("training/", "")
        cantilever_residual = UnderwaterCantileverResidualPhysics(config, save_folder, params)
        ...
        print(save_folder)
        cantilever_residual.train("cantilever_data_sim2sim_liquid", config)
