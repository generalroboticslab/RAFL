...
def test_trajectory(
    ...
    ground_truth = np.load(
        f"shorter_scaled_cantilever_data_sim2sim/optimized_data_{test_data_idx}.npy",
        allow_pickle=True,
    )[()]
    ...
    vis_1d_folder = f"shorter_scaled_sim2sim/{save_folder.replace('training/', '')}_{model_input}"
...
if __name__ == "__main__":
    ...
    cantilever = ShorterCantileverEnv3d(42, 'shorter_beam', hex_params)
    default_cantilever = CantileverEnv3d(42, 'shorter_beam', hex_params)
    ...
    os.makedirs(f"{save_folder}/shorter_scaled/visualizations/base", exist_ok=True)
    sim_errors = []
    res_errors = []
    ...
    generate_video_directory(f"{save_folder}/shorter_scaled/visualizations/residual", [2,7,11,14,16], flag="")
    generate_video_directory(f"{save_folder}/shorter_scaled/visualizations/base", [2,7,11,14,16], flag="")
