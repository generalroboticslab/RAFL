...
def train(
    cantilever:LongerCantileverEnv3d, save_folder, start_frame=0, end_frame=150, num_epochs=100, cantilever_sim=None, 
):
    ...
    with tqdm(num_epochs) as qbar:
        for epoch in range(num_epochs):
            ...
            for data_idx in training_options["training_set"]:
                qs_real = np.load(f'data_longer/q{data_idx}.npy' )
                ...
                q_arr = np.load(f"cantilever_data_longer_straight/q_force_opt{data_idx}_reorder.npz")['arr_%d' % (len(np.load(f"cantilever_data_longer_straight/q_force_opt{data_idx}_reorder.npz"))-1)]
...
if __name__ == "__main__":
    ...
    save_folder = f"training/test_refactor_element_nonWeighted_try_unseparated_unconditioned_longer_direct"
