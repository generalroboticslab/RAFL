...
def data_generate(sample_num, frame_num, hex_params, vis=True, target_dx=0.01):
    ...
    for idx in tqdm(range(sample_num)):
        ...
        Path(f"oscillate_solid_{target_dx}/{idx}").mkdir(parents=True, exist_ok=True)

        qs = [q.clone().detach().numpy()]
        vs = [v.clone().detach().numpy()]
        if vis:
            env.display_mesh(q, f"oscillate_solid_{target_dx}/{idx}/0.png")
        for k in range(int(frame_num)):
            ...
            q, v =  env.forward(q, v, dt=dt)
            if vis:
                file_name = f"oscillate_solid_{target_dx}/{idx}/{k+1}.png"
        ...
        data_folder = f'data_real_solid_{target_dx}'
        Path(f"{data_folder}").mkdir(parents=True, exist_ok=True)
        np.save(f"{data_folder}/trajectory{idx}.npy", data)
    
    generate_video_directory(f"oscillate_solid_{target_dx}", list(range(sample_num)), flag="")


if __name__ == "__main__":
    # youngs_modulus = 215856
    # poissons_ratio = 0.45
    youngs_modulus = 4000000 #9000000 
    poissons_ratio = 0.45 
    sample_num = 20 # 200
    frame_num = 100
    hex_params = {
