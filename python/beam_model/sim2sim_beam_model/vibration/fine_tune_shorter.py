    ...
    with tqdm(num_epochs) as qbar:
        for epoch in range(num_epochs):
            ...
            for data_idx in training_options["training_set"]:
                target_trajectory_q = torch.from_numpy(np.load(f"shorter_data_real/trajectory{data_idx}.npy", allow_pickle=True)[()]['q'])
                target_trajectory_v = torch.from_numpy(np.load(f"shorter_data_real/trajectory{data_idx}.npy", allow_pickle=True)[()]['v'])
