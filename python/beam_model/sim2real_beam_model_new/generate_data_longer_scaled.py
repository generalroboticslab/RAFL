...
def optimize_init_force(
    ...
    num_frames,
    target,
    cantilever: LongerCantileverEnv3d,
    dt,
    id=0,
    res_force=None,
    ...
    q_all = []
    print(f"test{id}")
    path_vis = Path(f"cantilever_data_longer_scaled_{suffix}/test{id}/init_beam")
    ...
    with torch.no_grad():
        ...
        Path(f"cantilever_data_longer_scaled_{suffix}").mkdir(exist_ok=True, parents=True)
        np.savez(f"cantilever_data_longer_scaled_{suffix}/q_force_opt{id}_reorder.npz", *q_all)
...
if __name__ == '__main__':
    ...
    for idx in idxs:
        ...
        cantilever = LongerCantileverEnv3d(42, f'cantilever_data_longer_scaled_straight/test{idx}', hex_params)
        ...
        qs_real = np.load(f"data_longer/q{idx}.npy")
