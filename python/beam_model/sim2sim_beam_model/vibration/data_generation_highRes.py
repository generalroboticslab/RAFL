...
from env_cantilever_highRes import HighResCantileverEnv3d
...
if __name__ == '__main__':
    ...
    cantilever = HighResCantileverEnv3d(42, save_folder, hex_params)
    ...
    for id in range(len(weights)):
        if save_folder == 'data_real':
            for j in tqdm(range(prepare)):
                ...
                res_force[-625*3+2::3] = - weight / 169
                q, v = cantilever.forward(q, v, f_ext=res_force, dt=0.01)
        elif save_folder == 'data_sim':
            q = torch.from_numpy(np.load(f'highRes_data_real/trajectory{id}.npy', allow_pickle=True)[()]['q'][0])
            v = torch.from_numpy(np.load(f'dhighRes_ata_real/trajectory{id}.npy', allow_pickle=True)[()]['v'][0])
        else:
