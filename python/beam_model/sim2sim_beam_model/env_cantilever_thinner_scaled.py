...
class ThinnerCantileverEnv3d (EnvBase):
    ...
    def __init__(self, seed, folder, options):
        ...
        ### Mesh Parameters
        # Cantilever is 0.10m long, 0.02m wide, and 0.02m tall
        dx = ndarray([0.01 / refinement, 0.0066667 / refinement, 0.0066667 / refinement])
        cell_nums = (int(round(0.1 / dx[0])), int(round(0.02 / dx[1])), int(round(0.02 / dx[2])))
        assert cell_nums[0] * dx[0] == 0.1 and np.isclose(cell_nums[1] * dx[1],0.02), "Refinement does not properly divide the cantilever dimensions!" 
