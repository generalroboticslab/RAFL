import sys
sys.path.append('../')
sys.path.append('../../..')
from pathlib import Path
import time
import numpy as np
import torch

# from env_cantilever import CantileverEnv3d
from env_cantilever_shorter_scaled import ShorterCantileverEnv3d

def optimize_trajectoryfull(cantilever:ShorterCantileverEnv3d, num_frames, num_epochs, dt, sample=0, suffix = "fix_registration"):
    # folder = 'trajectoryfull'
    # beam_folder = f'test{idx}'
    # Path(beam_folder+ '/'+ folder).mkdir(parents=True,exist_ok=True)
    # force_nodes_num = cantilever._dofs
    # params = torch.normal(0, 1e-4, [force_nodes_num, num_frames], dtype=torch.float64, requires_grad=True)
    target_trajectory_q = np.load(f"shorter_scaled_data_real/trajectory{sample}.npy", allow_pickle=True)[()]['q'][0]
    target_trajectory_v = np.load(f"shorter_scaled_data_real/trajectory{sample}.npy", allow_pickle=True)[()]['v'][0]
    # q0 = target_trajectory[0]
    ### Define target location
    # q_all = []
    # v_all = []
    # loss_history_init = []
    # loss_history_optimized = []
    # q_all.append(q0)
    # v_all.append(np.zeros_like(q0))

    # q_last_epoch = q0.clone()
    # v_last_epoch = torch.zeros(cantilever._dofs, dtype=torch.float64)
    # for frame_i in range(1, num_frames):
        # with torch.no_grad():
        #     params[:, frame_i] = params[:, frame_i - 1]
        # init_loss = ((target_trajectory[frame_i] - q_last_epoch)**2).sum().item()
        # loss_history_init.append(init_loss)
        # optimizer = torch.optim.Adam([params],lr=1e-3)
        # loss_last = 0
        #q_sim, v_sim = cantilever.forward(q_last_epoch, v_last_epoch, f_ext=torch.zeros_like(q_last_epoch), dt=dt)
        # for epoch in range(num_epochs):
        #     start_time = time.time()
        #     optimizer.zero_grad()

        #q, v = q_last_epoch.detach().clone(), v_last_epoch.detach().clone()
        #f_ext = torch.zeros(cantilever._dofs, dtype=torch.float64)
        #     f_ext[-force_nodes_num:] += params[:, frame_i]

        #q, v = cantilever.forward(q, v, f_ext=f_ext, dt=dt)
        #     data_loss = ((target_trajectory[frame_i] - q)**2).sum()
        #     loss = data_loss + 1e-4 * (params[:,frame_i]**2).sum()
        #     if (torch.abs(loss - loss_last)/loss < 1e-6):
        #         break
        #     loss_last = loss
        #     # Backward gradients so we know which direction to update parameters
        #     loss.backward()
        #     # Actually update parameters
        #     optimizer.step()
        # loss_history_optimized.append(loss.item())
        #v_last_epoch = v
        #q_last_epoch = q
        #v_all.append(v.detach().numpy())
        #q_all.append(q.detach().numpy())

        # print(f"Error sim and real: {((q.detach().numpy() - q_sim.detach().numpy())**2).sum()}")
    
        # with np.printoptions(precision=3):
        #     print(f"Frame [{frame_i}/{num_frames-1}]/ Epoch {epoch}: {(time.time()-start_time):.2f}s,- Loss {data_loss.item():.4e}, init_loss {init_loss} ")

    ### Plotting Loss History
    print("length of q_all", target_trajectory_q.shape[0])
    print("length of v_all", target_trajectory_v.shape[0])
    optimized_data_save = {
        "q_trajectory": target_trajectory_q,
        "v_trajectory": target_trajectory_v,
        # "optimized_forces": params.detach().numpy(),
    }
    Path(f"shorter_scaled_cantilever_data_{suffix}").mkdir(parents=True,exist_ok=True)
    np.save(f"shorter_scaled_cantilever_data_{suffix}/optimized_data_{sample}.npy", optimized_data_save)

if __name__ == '__main__':
    prepare = 100
    forward = 0
    forward_times = []
    transformed_data = []
    idxs = list(range(20))
    for idx in idxs:
        youngs_modulus = 215856
        poissons_ratio = 0.45
        density = 1.07e3
        state_force = [0, 0, -9.80709]
        hex_params = {
            'density': density,
            'youngs_modulus': youngs_modulus,
            'poissons_ratio': poissons_ratio,
            'state_force_parameters': state_force,
            'mesh_type': 'hex',
            'refinement': 1,
        }
        cantilever = ShorterCantileverEnv3d(42, f'test{idx}', hex_params)

        ########################################
        num_frames = 100
        num_epochs = 300
        optimize_trajectoryfull(cantilever, num_frames, num_epochs, 0.01, sample=idx, suffix="sim2sim")