...
def data_generate(sample_num, frame_num, hex_params, vis=True):
    ...
    oscillation_amplitudes = np.random.uniform(0.0005, 0.003, size=sample_num)
    for idx, amplitude in tqdm(enumerate(oscillation_amplitudes)):
        ...
        Path(f"oscillate_solid/{idx}").mkdir(parents=True, exist_ok=True)

        qs = [q0.clone().detach().numpy()]
        vs = [v0.clone().detach().numpy()]
        q, v = q0.clone(), v0.clone()
        if vis:
            env.display_mesh(q, f"oscillate_solid/{idx}/0.png")
        for k in range(int(frame_num)):
            ...
            if vis:
                file_name = f"oscillate_solid/{idx}/{k+1}.png"
        ...
        data_folder = 'data_real_solid'
        Path(f"{data_folder}").mkdir(parents=True, exist_ok=True)
        np.save(f"{data_folder}/trajectory{idx}.npy", data)
    
    generate_video_directory("oscillate_solid", list(range(sample_num)), flag="")


if __name__ == "__main__":
    # youngs_modulus = 215856
    # poissons_ratio = 0.45
    youngs_modulus = 9000000 #263824
