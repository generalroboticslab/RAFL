...
def optimize_trajectoryfull(cantilever:FishTailMeshEnv3d, num_frames, num_epochs, dt, sample=0, suffix = "fix_registration"):
    ...
    target_trajectory_q = np.load(f"fishTailMesh_data_real/trajectory{sample}.npy", allow_pickle=True)[()]['q'][0]
    target_trajectory_v = np.load(f"fishTailMesh_data_real/trajectory{sample}.npy", allow_pickle=True)[()]['v'][0]
    ...
    Path(f"fishTailMesh_data_{suffix}").mkdir(parents=True,exist_ok=True)
    np.save(f"fishTailMesh_data_{suffix}/optimized_data_{sample}.npy", optimized_data_save)
...
if __name__ == '__main__':
    ...
    for idx in idxs:
        ...
        cantilever = FishTailMeshEnv3d(42, f'fishTailMesh', hex_params)
