...
def test_trajectory(
    ...
    target_data = np.load(f"data_real_liquid_0.01/trajectory{test_data_idx}.npy", allow_pickle=True)[()]
    hex_params["amplitude"] = target_data["amplitude"]
    print(hex_params)
    cantilever = HangingCantileverScaledEnv3d(42, 'oscillate_base_0.01', hex_params, 0.01)
    cantilever_sim = HangingCantileverScaledEnv3d(42, 'oscillate_base_0.01', hex_params, 0.01)
    ...
    cantilever._q0,
    f"cantilever_data_sim2sim_liquid_0.01",
    start_frame=training_options["start_frame"],
    end_frame=training_options["end_frame"],
    )

    ground_truth = np.load(
        f"cantilever_data_sim2sim_liquid_0.01/optimized_data_{test_data_idx}.npy",
        allow_pickle=True,
    )[()]
    ...
    qs_sim = np.array(qs_sim) / cantilever_sim.s
    qs_res = np.array(qs_res) / cantilever.s
    qs_ground_truth =  np.array(target_data['q']) / cantilever.s #np.array(ground_truth["q_trajectory"])
...
if __name__ == "__main__":
    ...
    save_folder = f"training/test_refactor_0.01_liquid"
    sim_errors = []
    res_errors = []
    for test_i in range(12,20):
        youngs_modulus = 1000000 #97350 
        poissons_ratio = 0.3
        density = 6e3 
        hex_params = {
            'density': density,
