...
def test_trajectory(
    ...
    target_data = np.load(f"data_real_solid_0.01/trajectory{test_data_idx}.npy", allow_pickle=True)[()]
    ...
    cantilever._q0,
    f"cantilever_data_sim2sim_solid_0.01",
    start_frame=training_options["start_frame"],
    end_frame=training_options["end_frame"],
    )

    ground_truth = np.load(
        f"cantilever_data_sim2sim_solid_0.01/optimized_data_{test_data_idx}.npy",
        allow_pickle=True,
    )[()]
...
if __name__ == "__main__":
    ...
    save_folder = f"training/test_refactor_0.01_solid"
    sim_errors = []
    res_errors = []
