...
class HangingCantileverScaledEnv3d (EnvBase):
    ...
    def __init__(self, seed, folder, options, target_dx=0.01):
        ...
        ### Mesh Parameters
        # Cantilever is 0.0105m long, 0.0105m wide, and 0.105m tall
        dx = 0.0035
        self.s = target_dx / dx 
        cell_nums = (3, 3, 30)
        ...
        # Scale all physical parameters to simulation units
        density = density / (self.s**3)
        youngs_modulus = youngs_modulus / (self.s**2)
        self.amplitude = self.amplitude * self.s
        state_force_parameters = state_force_parameters * self.s
        ...
        generate_hex_mesh(voxels, target_dx, origin, bin_file_name)
    ...
    def display_mesh (self, q, file_name, extra_points=None):
        ...
        self._deformable.PySaveToMeshFile(ndarray(q) / self.s, tmp_bin_file_name)
